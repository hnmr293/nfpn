...
def to_hf8x(module: torch.nn.Module, convert_linear: bool = True, convert_conv2d: bool = False):
    target_modules = []
        ...
        for orig_class, hf_class in target_modules:
            if isinstance(mod, orig_class):
                try:
                    ...
                    print(f'[nfpn] *** WARN *** failed to convert module to HF8x: {name} {str(e)}')
                    break
...
def _get_param(data: torch.Tensor):
    if convert.HF8X_MAX <= data.abs().max():
        print('[nfpn] *** WARN *** max(abs(data)) >= HF8X_MAX')
    
    hf8x = convert.to_hf8x(data)
    
    hf8x.requires_grad_(False)
    
    hf8x = torch.nn.Parameter(hf8x, requires_grad=False)
...
class Linear(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        weight = convert.hf8x_to_fp16(*self.weight).reshape(self.weight_shape)
        bias = convert.hf8x_to_fp16(*self.bias).reshape(self.bias_shape) if self.bias else None
